...
Gileadcom is a dedicated headless application branched from the Forma Lux retail template. It uses the same industry-verticals component library but is isolated in its own rendering host so frontend and content changes do not affect forma-lux.
...
2. Starting from the root of the repository navigate to site app folder
   `cd industry-verticals/gileadcom`
...
6. Provide Editing host name `gileadcom` as per xmcloud.build.json
...
1. Use the existing forma-lux search source temporarily
2. Create a new source in the CEC portal and link it to your application

### Use existing forma-lux search source (temporary)
